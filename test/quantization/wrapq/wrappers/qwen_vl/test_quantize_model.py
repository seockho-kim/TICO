...
class TestQwenModelExample(unittest.TestCase):
    """Exercise the old quantize_model.py flow with a tiny synthetic model."""

    def test_prepare_convert_qwen3vl_model_flow_matches_example(self):
        """Quantize Qwen3VLModel and verify the quantized hidden-state output."""
        ...
        self.assertIsInstance(prepared.wrapped, QuantQwen3VLModel)
        self.assertIs(prepared._mode, Mode.CALIB)

        calibration_data = [
            _create_image_input(cfg, seq_len=50, thw=thw) for _ in range(2)
        ]
