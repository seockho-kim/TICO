...
@try_register("transformers.models.gemma4.modeling_gemma4.Gemma4VisionMLP")
class QuantGemma4VisionMLP(QuantModuleBase):
    """PTQ wrapper for Gemma4 vision MLP blocks."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Run the vision MLP with observers around activation math."""
        gate = self._fq(self.act_fn(self.gate_proj(x)), self.obs_gate_act)
        up = self.up_proj(x)
        return self.down_proj(self._fq(gate * up, self.obs_mul))
