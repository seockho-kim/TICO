...
def passes():
    ...
    return [
        ...
        RemoveRedundantPermutePattern2(),
        RemoveRedundantPermutePattern3(),
    ]
...
@trace_graph_diff_on_pass
class RemoveRedundantPermutePattern2(PassBase):
    def __init__(self):
        super().__init__()
    ...
    def call(self, exported_program: ExportedProgram) -> PassResult:
        """
        [BEFORE]
            (AxBxC) - aten.permute - aten.clone(contiguous) - aten.permute - (AxBxC)
        [AFTER]
            (AxBxC)
        """
        logger = logging.getLogger(__name__)
        ...
        for permute2 in graph.nodes:
            ...
            if len(permute2.users) != 1:
                continue
            assert len(permute2.args) == 2
            clone, permute2_dims = permute2.args
            assert isinstance(clone, torch.fx.Node), type(clone)
            assert isinstance(permute2_dims, list), type(permute2_dims)
            for dim in permute2_dims:
                assert isinstance(dim, int), type(dim)

            if clone.target not in ops.aten.clone:
                continue
            assert len(clone.args) == 1, len(clone.args)
            if "memory_format" not in clone.kwargs:
                continue
            if clone.kwargs["memory_format"] != torch.contiguous_format:
                continue

            permute1 = clone.args[0]
            assert isinstance(permute1, torch.fx.node.Node), type(permute1)
            if not permute1.target in ops.aten.permute:
                continue
            if len(permute1.users) != 1:
                continue
            ...
            modified = True
            logger.debug(
                f"{permute1.name}, {clone.name} and {permute2.name} are removed."
            )
...
@trace_graph_diff_on_pass
class RemoveRedundantPermutePattern3(PassBase):
    def __init__(self):
        super().__init__()
    ...
    def call(self, exported_program: ExportedProgram) -> PassResult:
        """
        [BEFORE]
            (AxBxC) - aten.permute - aten.permute - aten.clone(contiguous) - (AxBxC)
        [AFTER]
            (AxBxC)
        """
        logger = logging.getLogger(__name__)
        ...
        graph = graph_module.graph
        modified = False
        for clone in graph.nodes:
            if not clone.op == "call_function":
                continue
            if clone.target not in ops.aten.clone:
                continue
            assert len(clone.args) == 1, len(clone.args)
            ...
            permute2 = clone.args[0]
            assert isinstance(permute2, torch.fx.node.Node), type(permute2)
            if not permute2.target in ops.aten.permute:
                continue
            if len(permute2.users) != 1:
                continue
            ...
            modified = True
            logger.debug(
                f"{permute1.name}, {permute2.name} and {clone.name} are removed."
            )
